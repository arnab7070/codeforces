...
// Load the saved username from local storage if available
...
if (savedUsername) {
    ...
    // Show the alert message if the username key is not present or its value is empty
    document.getElementById('alert-message').classList.remove('d-none');
}
// Save the entered username to local storage on form submission
...
// Remove the user whenever sign-out is clicked
...
// To get the current topic selected by Code Now button
...
fetch('https://codeforces.com/api/user.info?handles=' + usernameInput.value)
    ...
    .then(data => {
        ...
        document.getElementById("avatar-container").src = imageUrl;
        document.getElementById("friends").textContent = friendOfCount;
...
// Make a GET request to the Codeforces API to get the list of contests
